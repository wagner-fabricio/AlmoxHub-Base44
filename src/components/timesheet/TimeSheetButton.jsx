...
export default function TimeSheetButton({
  ...
  if (estaPlayandoEu) {
    ...
    icon = <Pause className={isSmall ? 'w-3 h-3' : 'w-4 h-4'} />;
    tooltip = 'Pausar atividade';
    pulsar = true;
  } else if (algumEmPlay) {
    bgColor = 'bg-green-500 hover:bg-green-600 text-white';
    icon = <Play className={isSmall ? 'w-3 h-3' : 'w-4 h-4'} />;
  ...
  return (
    ...
    >
      {loading ? (
        <span className={`inline-block rounded-full border-2 border-white border-t-transparent animate-spin ${isSmall ? 'w-3 h-3' : 'w-3.5 h-3.5'}`} />
      ) : icon}
