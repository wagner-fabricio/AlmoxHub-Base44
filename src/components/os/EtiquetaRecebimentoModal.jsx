...
export default function EtiquetaRecebimentoModal({ open, onClose, os, subcategorias = [], almoxarifados = [] }) {
  ...
  const fornecedor    = os?.nfe_dados_emissor || {};
  const transportador = os?.nfe_dados_transportador || {};
  const almoxarifado  = almoxarifados?.find(a => a?.id === os?.almoxarifado_id);
  const subcatNomes   = (os?.subcategorias_ids || [])
    .map(id => subcatById(subcategorias, id)?.nome)
    .filter(Boolean).join(', ');

  const totalLabels = Math.max(1, parseInt(quantidade) || 1);

  const toggleSymbol = (id) =>
    setSelectedSymbols(prev => prev.includes(id) ? prev.filter(s => s !== id) : [...prev, id]);

  const fornecedorEndereco = () => {
    const f = fornecedor;
    return [f.endereco, f.numero, f.bairro,
      `${f.cidade || ''}${f.estado ? '/' + f.estado : ''}`, f.cep
    ].filter(Boolean).join(', ');
  };
  ...
  const buildPdf = async () => {
    ...
    const drawLabel = (lx, ly, lw, lh) => {
      ...
      const BOT_H    = Math.max(22,  lh   * 0.27);

      const colGap    = 1.5;
      const leftColW  = iw * 0.5 - colGap / 2;
      const rightColW = iw * 0.5 - colGap / 2;
      const colX_R    = lx + mg + leftColW + colGap;

      const TOP_H = lhOf(FSL) + 0.8 + lhOf(FSA) * 5 + 3.5;
      const MID_H = lh - HEADER_H - TOP_H - BOT_H;

      pdf.setLineWidth(0.8); pdf.setDrawColor(0);
      pdf.rect(lx, ly, lw, lh, 'S');

      // ══════════ HEADER (sem indicação de volume) ══════════
      ...
      const recLabel = 'RECEBIMENTO';
      const recW = pdf.getStringUnitWidth(recLabel) * FSH / pdf.internal.scaleFactor;
      pdf.text(recLabel, lx+lw-mg-recW, ly + HEADER_H*0.74);
      pdf.setTextColor(0,0,0);

      // ══════════ TOP — Fornecedor / Transportador ══════════
      ...
      const renderBlock = (nome, infoLines, x, startY, colW) => {
        const fullText = [nome, ...infoLines].filter(Boolean).join('\n');
        ...
        const restLines = pdf.splitTextToSize(infoLines.filter(Boolean).join('\n'), colW);
        const allLines  = [...nameLines, ...restLines].slice(0, maxLines);
        let lineIdx = 0;
      ...
      pdf.text('FORNECEDOR:', lx+mg, ty + lhOf(FSL)*0.76);
      ty += lhOf(FSL) + 0.8;
      renderBlock(
        fornecedor.razao_social || '-',
        [
          fornecedor.cnpj ? `CNPJ: ${fornecedor.cnpj}` : '',
          fornecedor.inscricao_estadual ? `IE: ${fornecedor.inscricao_estadual}` : '',
          fornecedorEndereco(),
        ],
        lx+mg, ty, leftColW
      );
      ...
      // Right: TRANSPORTADOR
      pdf.setFont('helvetica','bold'); pdf.setFontSize(FSL);
      pdf.text('TRANSPORTADOR:', colX_R, ry + lhOf(FSL)*0.76);
      ry += lhOf(FSL) + 0.8;
      renderBlock(
        transportador.razao_social || '-',
        [
          transportador.cnpj ? `CNPJ: ${transportador.cnpj}` : '',
          transportador.inscricao_estadual ? `IE: ${transportador.inscricao_estadual}` : '',
          transportador.endereco || '',
          (transportador.quantidade_volumes || transportador.especie_volume)
            ? `Volumes: ${transportador.quantidade_volumes || '—'}${transportador.especie_volume ? ` (${transportador.especie_volume})` : ''}`
            : '',
        ],
        colX_R, ry, rightColW
      );
      ...
      // ══════════ MIDDLE — Ordem de Serviço + Recebimento ══════════
      const midStart = topEnd;
      const midEnd   = midStart + MID_H;
      const hasSyms  = selectedSymbols.length > 0;
      const symZoneW = hasSyms ? Math.min(lw * 0.33, 36) : 0;
      const dataW    = iw - symZoneW;
      let my = midStart + 1.5;

      const dataMigo = os?.data_migo_receb ? ` (${os.data_migo_receb})` : '';
      const midLines = [
        { text: 'ORDEM DE SERVIÇO:', bold: true, big: false },
        { text: os?.codigo || '-',   bold: true, big: true  },
        ...(os?.nfe_numero_receb  ? [{ text: `NF: ${os.nfe_numero_receb}`, bold: false }] : []),
        ...(os?.numero_migo_receb ? [{ text: `MIGO: ${os.numero_migo_receb}${dataMigo}`, bold: false }] : []),
        ...(os?.numero_v360       ? [{ text: `ID V360: ${os.numero_v360}`, bold: false }] : []),
        { text: '---divider---', bold: false },
        ...(os?.data_recebimento      ? [{ text: `Data Receb.: ${os.data_recebimento}`, bold: false }] : []),
        ...(os?.responsavel_recebimento ? [{ text: `Resp. Receb.: ${os.responsavel_recebimento}`, bold: false }] : []),
      ];

      const midFsMax  = n === 4 ? FSS * 0.82 : FSS;
      const midAvailH = MID_H - 3;
      ...
      // ══════════ BOTTOM — QR (left) | Dados Gerais (right) ══════════
      const botY   = midStart + MID_H;
      const qrSize = Math.min(BOT_H - 3, iw * 0.34);
      const qrX    = lx + mg;
      const qrY    = botY + (BOT_H - qrSize) / 2;

      if (qrDataUrl) pdf.addImage(qrDataUrl, 'PNG', qrX, qrY, qrSize, qrSize);
      ...
      if (bcImg) {
        ...
        const infoItems = [
          { label: 'Subcategoria', value: subcatNomes || '-' },
          { label: 'Almoxarifado', value: almoxarifado?.nome || '-' },
          { label: 'Anotações',    value: os?.descricao_resumida || '-', wrap: true },
        ];
    ...
    for (let i = 0; i < totalLabels; i++) {
      const pos = i % n, col = pos % cols, row = Math.floor(pos / cols);
      if (i > 0 && pos === 0) pdf.addPage();
      const lx = MG + col*(LW+GAP);
      const ly = MG + row*(LH+GAP);
      drawLabel(lx, ly, LW, LH);
    }
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.save(`Etiquetas_Recebimento_${os?.codigo || 'OS'}.pdf`);
    } catch (err) {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto" aria-describedby={undefined}>
          ...
          <div className="flex items-center gap-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl p-3 border border-blue-200 dark:border-blue-800">
            ...
            <div>
              ...
              <p className="text-xs text-blue-700 dark:text-blue-300">
                Modelo Transportadora · OS {os?.codigo}
              </p>
            </div>
          </div>

          <div className="space-y-2">
            <Label className="text-sm font-semibold">Quantidade de Etiquetas</Label>
            <Input
              type="number"
              min="1"
              value={quantidade}
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Modelo completo com QR, código de barras da OS, símbolos ISO e dados de recebimento.
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-3 border-t mt-2">
          ...
          <Button variant="outline" onClick={handlePrint} disabled={generating} className="sm:flex-1">
            {generating
              ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Gerando...</>
              : <><Printer className="w-4 h-4 mr-2" />Imprimir</>}
          </Button>
          <Button onClick={handleDownload} disabled={generating} className="sm:flex-1" style={{ backgroundColor: '#0000FF' }}>
            {generating
              ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Gerando...</>
...
function subcatById(subcategorias, id) {
